...
import { getBarChart,getLineChart,getPieChart,getNPMDependencies } from "../formComponents/charts";
...
export default class DragLayout extends PureComponent {
  ...
  generateDOM = () => {
    ...
    return _.map(this.state.widgets, (l, i) => {
      let option;
      if (l.type === 'bar') {
        console.log('getBarChart',getBarChart())
        option = getBarChart();
      }else if (l.type === 'line') {
        option = getLineChart();
      }else if (l.type === 'pie') {
        option = getPieChart();
      }else if (l.type === 'dependencies'){
        option = getNPMDependencies();
      }
      let component = (
        <ReactEcharts
          option={option}
   ...
   return(
     <Layout>
      <Header style={{ position: 'fixed', zIndex: 1, width: '100%','padding': '0 30px' }}>
        ...
        <Button type="primary" style={{'marginRight':'7px'}} onClick={this.addChart.bind(this,'dependencies')}>添加依赖关系图</Button>
      </Header>
